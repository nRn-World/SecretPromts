...
export const Marquee: React.FC = () => {
  ...
  return (
    <div className="relative w-full overflow-hidden bg-zinc-950/60 py-4 border-b border-zinc-800/40">
        ...
        {tickerItems.map((prompt, index) => {
          ...
          return (
            <div
              ...
              className="relative flex-shrink-0 w-32 h-32 sm:w-40 sm:h-40 rounded-xl overflow-hidden cursor-pointer group bg-zinc-900 border border-zinc-800 transition-transform duration-300 hover:scale-105 hover:z-10 shadow-md"
              title={title}
            >
              <img
                src={prompt.imageUrl}
                alt={title}
                className="w-full h-full object-contain bg-zinc-800 transition-transform duration-500 group-hover:scale-110"
                loading="lazy"
              />
